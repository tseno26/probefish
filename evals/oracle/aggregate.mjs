...
function summarize(arm) {
  ...
  return {
    arm,
    runs: armRuns.length,
    meanTrapsPassed: round(mean(armRuns.map((r) => r.trapsPassedCount))),
    trapsTotal,
    meanDurationSec: round(mean(armRuns.map((r) => r.durationSec ?? 0)), 1),
    meanOwnProbesPlanted: round(mean(armRuns.map((r) => r.ownProbesPlanted ?? 0))),
  };
}
...
console.log('');
console.log(`=== Summary (mean across ${args.n} runs per arm) ===`);
console.log(`${pad('arm', 12)}${padNum('runs', 8)}${padNum('mean traps ok', 18)}${padNum('mean secs', 12)}${padNum('mean own-probes', 18)}`);
for (const s of [summary['no-skill'], summary.probefish]) {
  console.log(
    `${pad(s.arm, 12)}${padNum(s.runs, 8)}${padNum(`${s.meanTrapsPassed}/${s.trapsTotal}`, 18)}${padNum(s.meanDurationSec, 12)}${padNum(s.meanOwnProbesPlanted, 18)}`,
  );
}
console.log('');
